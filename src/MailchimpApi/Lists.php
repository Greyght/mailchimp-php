...
though you should cap them at 5k - 10k records, depending on your experience. These calls are also long, so be sure you increase your timeout values.
     * @param string $id
     * @param array $batch
     *     - email array a struct with one of the following keys - failing to provide anything will produce an error relating to the email address. Provide multiples and we'll use the first we see in this same order.
     ...
     *     - merge_vars array data for the various list specific and special merge vars documented in lists/subscribe
     * @param boolean $double_optin
     * @param boolean $update_existing
     * @param boolean $replace_interests
     * @return array struct of result counts and associated data
     ...
     *         - row array the row from the batch that caused the error
     */
     ...
     *     - desktop array desktop user agents and percentages
     ...
     *     - mobile array mobile user agents and percentages
...
grouping will automatically turn them on.
     ...
     *             - campaign_data array If not deleted, the campaigns/list data for the campaign
     */
     ...
     *     - error_count int the number of subscribers who were not found on the list
     *     - errors array array of error structs including:
     *         - email array whatever was passed in the email parameter
     *             - email string the email address added
     *             - euid string the email unique id
     *             - leid string the list member's truly unique id
     *         - error string the error message
     ...
     *         - merges array a struct containing a key for each merge tags and the data for those tags for this email address, plus:
     ...
     *         - geo array the geographic information if we have it. including:
     ...
     *         - clients array the client we've tracked the address as using with two keys:
...
data or specific members of a list? If so, checkout the <a href="/export/1.0/list.func.php">List Export API</a>
     ...
     *     - segment array a properly formatted segment that works with campaigns/segment-test
     * @return array of the total records matched and limited list member data for this page
     ...
     * @return array the full data for the new merge var, just like merge-vars returns
...
used.
     ...
     *     - segment_opts array for "saved" only, the standard segment match+conditions, just like campaigns/segment-test
...
in order to be removed - this <strong>will not</strong> unsubscribe them from the list!
     ...
     *     - mc_location array Set the member's geographic location either by optin_ip or geo data.
...
consider using lists/batch-subscribe() with the update_existing and possible replace_interests parameter.
     ...
     *     - actions array the possible actions and whether they are enabled
     ...
     * @return array result of the operation including valid data and any errors
     ...
     *         - stats array various stats and counts for the list - many of these are cached for at least 5 minutes
