...
class Campaigns {
     ...
     * @return array containing all content for the campaign
     ...
     *     - tracking array optional - set which recipient actions will be tracked. Click tracking can not be disabled for Free accounts.
     ...
     *     - analytics array optional - one or more of these keys set to the tag to use - that can be any custom text (up to 50 bytes)
     ...
     *     - crm_tracking array optional If set, a struct to enable CRM tracking for:
     *         - salesforce array optional Enable SalesForce push back
     ...
     *         - highrise array optional Enable Highrise push back
     *             - campaign bool optional - if true, create a Kase object and update it with aggregate stats
     *             - notes bool optional - if true, attempt to update Contact notes based on email address
     *         - capsule array optional Enable Capsule push back (only notes are supported)
     *             - notes bool optional - if true, attempt to update Contact notes based on email address
     * @param array $content
     *     - html string for raw/pasted HTML content
     *     - sections array when using a template instead of raw HTML, each key should be the unique mc:edit area name from the template.
     ...
     *     - rss array For RSS Campaigns this, struct should contain:
     ...
     *         - days array optional used for "daily" schedules only, an array of the <a href="http://en.wikipedia.org/wiki/ISO-8601#Week_dates" target="_blank">ISO-8601 weekday numbers</a> to send on
     ...
     *     - absplit array For A/B Split campaigns, this struct should contain:
     ...
     *     - auto array For AutoResponder campaigns, this struct should contain:
     ...
     *             - 6 bool optional Saturday, defaults to true
     *             - 7 bool optional Sunday, defaults to true
     * @return array the new campaign's details - will return same data as single campaign from campaigns/list()
     */
     ...
     * @return array containing a count of all matching campaigns, the specific ones for the current page, and any errors from the filters provided
     ...
     *         - tracking array the various tracking options used
     ...
     *         - saved_segment array if a saved segment was used (match+conditions returned above):
     *             - id int the saved segment id
     *             - type string the saved segment type
     *             - name string the saved segment name
     *         - type_opts array the type-specific options for the campaign - can be passed to campaigns/create()
     ...
     *         - summary array if available, the basic aggregate stats returned by reports/summary
     *         - social_card array If a social card has been attached to this campaign:
     ...
     * @param string $cid
     * @return array the matching campaign's details - will return same data as single campaign from campaigns/list()
     */
...
a campaign is using. You only want to use this if you want to allow editing template sections in your application.
     * @param string $cid
     * @return array content containing all content section for the campaign - section name are dependent upon the template used and thus can't be documented
     */
...
<li>To clear/unset segment_opts, pass an empty string or array as the value. Various wrappers may require one or the other.</li>
     ...
     * @param string $name
     * @param array $value
     * @return array updated campaign details and any errors
     *     - data array the update campaign details - will return same data as single campaign from campaigns/list()
